...
export default {
  data() {
    return {
      userId: this.$route.params.id, // 当前页面的ueser id
      EmployeeEnum,
  ...
  methods: {
    ...
    async getDetailedUserInfo() {
      ...
      //如果调用来的userinfo里面有图片 就把图片的地址给上传框实例的fileList,同时标记upload为true
    ...
    async saveUserDetailById() {
      //先判断是否fileList中的upload都为trur ，如果有不为true的则不给保存
      const file = this.$refs.userPhoto.fileList
      if (file.some(item => !item.upload)) {
        //找到了说明有还没上传成功的。
      ...
      await saveUserDetailById({
        ...this.userInfo,
        staffPhoto: file && file.length ? file[0].url : ' '
      })
    ...
    async getPersonalDetail() {
      ...
      if (this.formData.staffPhoto && this.formData.staffPhoto.trim()) {
        this.$refs.personalPhoto.fileList = [
          {
    ...
    async updatePersonal() {
      const file = this.$refs.personalPhoto.fileList
      if (file.some(item => !item.upload)) {
        this.$message.warning('您当前还有图片没有上传完成！')
      }
